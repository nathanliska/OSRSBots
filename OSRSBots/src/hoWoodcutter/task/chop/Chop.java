...
public class Chop extends Node {
	...
	@Override
	public void execute() {
		//if(script.getSettings().getLocations().getTreeArea().getArea().contains(*Tree That Exists*)) //trying to check if the area has a tree in it that isn't cut down
		...
		if(tree.interact("Chop down") && script.getSettings().getLocations().getTreeArea().getArea().contains(tree.getTile())) {
			AbstractScript.sleepUntil(() -> !tree.exists() || script.getInventory().isFull() || script.getLocalPlayer().isInCombat(), //need to find a way to add onlevelup and onrandom
